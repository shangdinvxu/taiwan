...
import com.example.android.bluetoothlegatt.bean.LpHeartrateData;
...
//	public static boolean IS_DESTORY = false;
    ...
    public static void setCC_QIABBAO(boolean cC_QIABBAO) {
        CC_QIABBAO = cC_QIABBAO;
    }
...
//                if (!CommonUtils.isStringEmpty(MyApplication.getInstance(BleService.this).getLocalUserInfoProvider().getDeviceEntity().getLast_sync_device_id()) && !isCC_QIABBAO()) {
//                    MyLog.e(TAG, "getLast_sync_device_id：" + MyApplication.getInstance(BleService.this).getLocalUserInfoProvider().getDeviceEntity().getLast_sync_device_id() + "");
//                    MyLog.e(TAG, "isNEED_SCAN()：" + isNEED_SCAN());
//                    //	获得用户设备信息非空则：将设备信息数据添加进当前设备MAC中。
//                    if(!provider.isConnectedAndDiscovered()){
//                        provider.setCurrentDeviceMac(MyApplication.getInstance(BleService.this).getLocalUserInfoProvider().getDeviceEntity().getLast_sync_device_id());
//                        // 若蓝牙是断开的 才去扫描并且重连
...
//			{    //	获得用户设备信息非空则：将设备信息数据添加进当前设备MAC中。
//				provider.setCurrentDeviceMac(MyApplication.getInstance(BleService.this).getLocalUserInfoProvider().getLast_sync_device_id());
//				// 若蓝牙是断开的 才去扫描并且重连
//				if (!provider.isConnectedAndDiscovered()){
//					provider.scanForConnnecteAndDiscovery();
//				}
...
//		broadcastUpdate(SERVICE_DESTORY, BleService.this);
    ...
    private BLEProvider initBLEProvider() {
        //实现协议LepaoProtocalImpl：协议作为提供者的参数。
...
//                MyApplication.getInstance(BleService.this).getLocalUserInfoProvider().getDeviceEntity().setLast_sync_device_id(null);
            ...
            protected void notifyforsendGoalSuccess(){
                MyLog.e(TAG, "发送运动目标成功"+provider.isConnectedAndDiscovered());
//                MyToast.show(BleService.this, "发送运动目标成功", Toast.LENGTH_SHORT);
                super.notifyforsendGoalSuccess();
            }
            protected  void notifyforsendGoalFaild(){
                MyLog.e(TAG, "发送运动目标失败");
//                MyToast.show(BleService.this,"发送运动目标失败",Toast.LENGTH_SHORT);
                super.notifyforsendGoalSuccess();
...
//        bleSynchronImpl.synchronAll();
        MyLog.i(TAG, "同步所有设备信息：获得所有设备新的信息+用户信息userid:"+userEntity.getUser_id());
    }
            ...
            mBroadcastReceiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    if (MyApplication.getInstance(BleService.this).getLocalUserInfoProvider().getDeviceEntity() != null) {
                        ...
                        if (!CommonUtils.isStringEmpty(MyApplication.getInstance(BleService.this).getLocalUserInfoProvider().getDeviceEntity().getLast_sync_device_id()) && isNEED_SCAN() && !isCANCLE_ANCS()
                                && MyApplication.getInstance(BleService.this).getLocalUserInfoProvider().getDeviceEntity().getDevice_type()==MyApplication.DEVICE_WATCH) {   //蓝牙关闭 和 OAD升级过程中 取消定时器
                            MyLog.e(TAG, "系统当前时间:" + System.currentTimeMillis() / 1000 + "  扫描器当前时间:" + timer + "  差值:" + (System.currentTimeMillis() / 1000 - timer));
