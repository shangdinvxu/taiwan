...
import com.linkloving.taiwan.utils.sportUtils.Datahelper;
...
public class CalDataActivity extends ToolBarActivity implements View.OnClickListener {
    ...
    private int CALORIES_EVERY_DAY  ;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CALORIES_EVERY_DAY = getCalories();
...
//            SimpleDateFormat sim2 = new SimpleDateFormat("yyyy-MM");
    ...
    private BarData getDayBarData() {
        ...
        //一天有48条数据,要加上基础值,每个时间段的基础值是不一样的,一天总共是基础值是CALORIES_EVERY_DAY,
        //计算出每半个小时的基础值
        if (data == null) {
            //每段时间都是基础值,不加消耗的
...
//                        + _Utils.calculateCalories(Double.parseDouble(mDaySynopic.getWork_distance())/ (Double.parseDouble(mDaySynopic.getWork_duration())),Integer.parseInt(mDaySynopic.getWork_duration()), userEntity.getUserBase().getUser_weight());
                    ...
                    if (calendar.get(c.DAY_OF_WEEK) == c.get(c.DAY_OF_WEEK)) {
                        ...
                        //不是当前天
                        MyLog.i(TAG, "不是当前天=" + calValue + CALORIES_EVERY_DAY);
                        if ((c.get(Calendar.DAY_OF_WEEK) - 1) == 0) {
                            entries.add(new BarEntry(calValue + CALORIES_EVERY_DAY, 6));
                            list.add(6);
                        } else {
                            entries.add(new BarEntry(calValue + CALORIES_EVERY_DAY, c.get(c.DAY_OF_WEEK) - 2));
            ...
            for (int j = 0; j < calendar.get(Calendar.DAY_OF_WEEK) - 1; j++) {
                if (list.contains(j)) {
                    ...
                    if (j == calendar.get(Calendar.DAY_OF_WEEK) - 2) {
                        ...
                        entries.add(new BarEntry(CALORIES_EVERY_DAY, j));
                        continue;
                    }
                }
            }
...
//                            + _Utils.calculateCalories(Double.parseDouble(mDaySynopic.getWork_distance())/ (Double.parseDouble(mDaySynopic.getWork_duration())),Integer.parseInt(mDaySynopic.getWork_duration()), userEntity.getUserBase().getUser_weight());
                    MyLog.i(TAG, "卡路里=" + calValue);
                    entries.add(new BarEntry(calValue + CALORIES_EVERY_DAY, s - 1));//计算出,这条记录是在哪一个下标
                }
...
//                        + _Utils.calculateCalories(Double.parseDouble(mDaySynopic.getWork_distance())/ (Double.parseDouble(mDaySynopic.getWork_duration())),Integer.parseInt(mDaySynopic.getWork_duration()), userEntity.getUserBase().getUser_weight());
    ...
    public int getCalories() {
        ...
        int localUnit = SwitchUnit.getLocalUnit(CalDataActivity.this);
        int dayCalories = 0;
        ...
        if (dayButton.isChecked()) {
            ...
            if (daySynopicList == null || daySynopicList.size() <= 0) {
                ...
                if (ToolKits.getBetweenDay(date1, new Date()) == 0) {
                    ...
                    calValuecount = CALORIES_EVERY_DAY;
                }
                ...
                if (ToolKits.getBetweenDay(date1, new Date()) == 0) {
                    //是今天
                    calValuecount = cal + calValue;
                } else {
                    calValuecount = calValue + CALORIES_EVERY_DAY;
                }
            }
        ...
        for (DaySynopic myDaySynopic : mDaySynopicArrayList) {
            ...
            try {
                ...
                if (i > 0) {
                    ...
                    int calValue = runcal + walkcal;
                    mycal.setCalory(calValue + CALORIES_EVERY_DAY);
    ...
    /* private  ArrayList<DaySynopic> querydata(int position){
         if(dayButton.isChecked()){
             ...
             Date date1=ToolKits.getDayFromDate(new Date(0), position + 1);
             //查询这天的数据
             final String time=new SimpleDateFormat(ToolKits.DATE_FORMAT_YYYY_MM_DD).format(date1);
             MyLog.e(TAG, "time=" + time);
             ArrayList<DaySynopic> mDaySynopicArrayList= DaySynopicTable.findDaySynopicRange(CalDataActivity.this, userEntity.getUser_id()+"", time, time, String.valueOf(TimeZoneHelper.getTimeZoneOffsetMinute()));
            ...
            *//* int calValue=0;
            if(mDaySynopic!=null)
            {
                ...
                int runcal =  _Utils.calculateCalories(Double.parseDouble(mDaySynopic.getRun_distance())/(Double.parseDouble(mDaySynopic.getRun_duration())),(int)runtime*60,userEntity.getUserBase().getUser_weight());
                int walkcal =  _Utils.calculateCalories(Double.parseDouble(mDaySynopic.getWork_distance())/(Double.parseDouble(mDaySynopic.getWork_duration())),(int)walktime*60,userEntity.getUserBase().getUser_weight());
                calValue=runcal+walkcal;
                //查到数据
                MyLog.e(TAG,"//查到数据"+mDaySynopic.toString());

                MyLog.e(TAG,"这天的数据calValue="+calValue);

            }else {
                MyLog.e(TAG,"这天的数据是空的"+mDaySynopic.toString());
            }

            MyLog.e(TAG, "两个日期相差多少天=" + ToolKits.getBetweenDay(new Date(), new Date()));
            if(ToolKits.getBetweenDay(date1,new Date())==0){
                //是今天
                calValue=cal+calValue;
            }else {
                calValue=calValue+CALORIES_EVERY_DAY;
            }
        ...
        //周数据
            ...
            MyLog.i(TAG, "周mDaySynopicArrayList=" +mDaySynopicArrayList.size());


            return mDaySynopicArrayList;
           *//* int countcalValue=0;
            //判断是不是本周,如果是本周
            if(day2.equals(getString(R.string.thisweek))){
                //是本周
                Calendar calendar=Calendar.getInstance();
                calendar.setTime(new Date());
                int day=calendar.get(Calendar.DAY_OF_WEEK)-1;//当前是星期几
                //没有查到数据
                if(mDaySynopicArrayList.size()<=0) {
                    countcalValue = CALORIES_EVERY_DAY * (day-1)+cal;
                }else {
                    for (DaySynopic mDaySynopic:mDaySynopicArrayList){
                        ...
                        int walkcal =  _Utils.calculateCalories(Double.parseDouble(mDaySynopic.getWork_distance())/(Double.parseDouble(mDaySynopic.getWork_duration())),(int)walktime*60,userEntity.getUserBase().getUser_weight());
                        calValue=runcal+walkcal;
                        countcalValue=countcalValue+calValue;
                    }
                    //在加上每天的基础量
                    countcalValue=countcalValue+CALORIES_EVERY_DAY * (day-1)+cal;
                }
            }else {
                //没有查到数据
                if(mDaySynopicArrayList.size()<=0){
                    countcalValue=CALORIES_EVERY_DAY*7;
                }else {
                    for (DaySynopic mDaySynopic:mDaySynopicArrayList){
                        int calValue=0;
                        //走路 分钟
                    ...
                    countcalValue=countcalValue+CALORIES_EVERY_DAY*7;//加上一周的基础量
                }
            }
            return countcalValue;*//*
        }
        //月数据
        else{
            SimpleDateFormat sim1 = new SimpleDateFormat("MM");
            SimpleDateFormat sim3 = new SimpleDateFormat("yyyy");
            int year = Integer.parseInt(sim3.format(new Date()));
            int mm = Integer.parseInt(sim1.format(new Date()));//当前时间的月份
            int month = (year - 1970) * 12 + mm + 1;
            ...
            //计算这个月有多少天
            Calendar c = Calendar.getInstance();
            Date date = ToolKits.stringToDate(startDateString, ToolKits.DATE_FORMAT_YYYY_MM_DD);
            c.setTime(date);
            ...
            MyLog.i(TAG, "月mDaySynopicArrayList=" +mDaySynopicArrayList.size());
            return mDaySynopicArrayList;
        }
          *//*  int countcalValue=0;
            if(day2.getText().equals(getString(R.string.thismonth))){
                ...
                int day=Integer.parseInt(sim.format(new Date()));//当前日期是几号,
                if(mDaySynopicArrayList.size()<=0) {
                    countcalValue = day * CALORIES_EVERY_DAY;
                }else {
                    ...
                    countcalValue=countcalValue+day*CALORIES_EVERY_DAY;
                }
            }else {
                //不是本月
                //没有数据
                if(mDaySynopicArrayList.size()<=0){
                    countcalValue=days*CALORIES_EVERY_DAY;
                }else {
                    ...
                    countcalValue=countcalValue+days*CALORIES_EVERY_DAY;
                }
...
//            monthposition=position;
    ...
    private class MyCalAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            if (calory != null && calory.getCalory() != 0)
