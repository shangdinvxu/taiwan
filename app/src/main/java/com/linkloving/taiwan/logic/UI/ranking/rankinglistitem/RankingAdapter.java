package com.linkloving.taiwan.logic.UI.ranking.rankinglistitem;
...
//                .showImageOnFail(R.mipmap.default_avatar)//加载失败显示图片
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof HeadViewHolder) {
            ...
            final Bitmap bmp_woman = BitmapFactory.decodeResource(res, R.mipmap.default_avatar_m);
            final Bitmap bmp_man = BitmapFactory.decodeResource(res, R.mipmap.default_avatar);
            imageLoader.displayImage(url, headViewHolder.ranking_activity_listview_item_imageView_head, options, new ImageLoadingListener() {
                ...
                public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
                    ...
                    if(U.getUserBase().getUser_sex()==0){
                        //女生
                        headViewHolder.ranking_activity_listview_item_imageView_head.setImageBitmap(bmp_woman);
                    }else {
                        //男生
                        headViewHolder.ranking_activity_listview_item_imageView_head.setImageBitmap(bmp_man);
                    }
                }
                    ...
                    public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
                        if (rholder.ranking_user_head.getTag().equals(imageUri)) {
                            ...
                            if(Integer.parseInt(rankingVOs.get(position - 1).getUser_sex())==0){
                                ...
                                rholder.ranking_user_head.setImageResource(R.mipmap.default_avatar);
                            }
                        }
                    }
                    @Override
