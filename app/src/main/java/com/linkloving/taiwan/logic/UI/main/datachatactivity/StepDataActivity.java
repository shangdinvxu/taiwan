...
public class StepDataActivity extends ToolBarActivity implements View.OnClickListener {
    ...
    Map<Integer,ArrayList<SportRecord>> data=new HashMap<>();//日数据查询结果
    ...
    protected void getIntentforActivity() {
        ...
        timenowdata=data=new Datahelper(StepDataActivity.this, String.valueOf(userEntity.getUser_id()), timeNow, new SimpleDateFormat(ToolKits.DATE_FORMAT_YYYY_MM_DD).format(date),false).getMydata();
    }
...
//        btn.setBackground(ContextCompat.getDrawable(StepDataActivity.this, R.mipmap.btn_share));
    ...
    private BarData getDayBarData() {
        ...
        //一天有48条数据
        for(int i=0;i<48;i++){
          entries.add(new BarEntry((float) getstep(data.get(i)),i));
         }
...
//        dataSet.setColor(getResources().getColor(R.color.yellow_title));
    ...
    private BarData getWeekBarData() {
        ...
        //一周7条数据
         MyLog.e(TAG,"获得数据" +listData.size());
            ...
            c.setTime(date);
            String endDateString=new SimpleDateFormat(com.linkloving.taiwan.utils.ToolKits.DATE_FORMAT_YYYY_MM_DD).format(date);
        ...
        if(dayButton.isChecked()){
            AsyncTask myDayAsync=new AsyncTask<Object,Object,List<DaySynopic>>(){
                ...
                protected List<DaySynopic> doInBackground(Object... params) {
                    ...
                    String time=new SimpleDateFormat(com.linkloving.taiwan.utils.ToolKits.DATE_FORMAT_YYYY_MM_DD).format(date1);
    ...
    private void setDataToView(int position,List<DaySynopic> daySynopicList){
        if(dayButton.isChecked()){
            ...
            TextView title= (TextView) view.findViewById(R.id.title);
            title.setText(getString(R.string.Step) + "(" + getString(R.string.unit_step) + ")");
   ...
   /* private ArrayList<DaySynopic> querydata(int position){
        if(dayButton.isChecked()){
            ...
            ArrayList<DaySynopic> mDaySynopicArrayList= DaySynopicTable.findDaySynopicRange(StepDataActivity.this, userEntity.getUser_id()+"", time, time, String.valueOf(TimeZoneHelper.getTimeZoneOffsetMinute()));
