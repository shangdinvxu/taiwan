...
public class NotificationCollectorService  extends NotificationListenerService{
	...
	public void onCreate() {
		super.onCreate();
		MyLog.i( TAG ,  "NotificationCollectorService--onCreate()了" );
	}
	...
	public void onNotificationPosted(StatusBarNotification sbn) {
		 ...
		 MyLog.i(TAG ,"mNotification:"+sbn.toString());
		 provider = BleService.getInstance(NotificationCollectorService.this).getCurrentHandlerProvider();
	     if(mNotification!=null && provider!=null && provider.isConnectedAndDiscovered()){ 																		//OAD的时候取消一切其他蓝牙命令
	    	 if(!CommonUtils.isStringEmpty(userEntity.getDeviceEntity().getLast_sync_device_id()) && !BleService.isCANCLE_ANCS() && userEntity.getDeviceEntity().getDevice_type()==MyApplication.DEVICE_WATCH){
	    		 Bundle extras = mNotification.extras;
				 if(extras==null) return;
				 DeviceSetting deviceSetting = LocalUserSettingsToolkits.getLocalSetting(NotificationCollectorService.this, MyApplication.getInstance(NotificationCollectorService.this).getLocalUserInfoProvider().getUser_id()+"");
				 MyLog.i(TAG ,"deviceSetting.getANCS_value():"+deviceSetting.getANCS_value());
